...
bot.onText(/\/restart/, async (msg) => {
  ...
  // 1️⃣ Premium check
  if (!isPremium(userId)) {
    return bot.sendMessage(chatId, "🚫You are not a premium user. Please contact the owner to buy premium access. @syed_hacker_official to use /restart command!");
  }
...
bot.onText(/\/reqpair(?:\s*(.+))?/, async (msg, match) => {
  ...
  // Premium check
  if (!isPremium(telegramId)) {
    return bot.sendMessage(chatId, "🚫 You are not a premium user. Please contact the owner to buy premium access. @syed_hacker_official");
  }
...
║ premium access. @syed_hacker_official
║
╚═════════════════╝`);
  }
...
║
║ You are not a premium user.
║ Contact owner to get premium access. @syed_hacker_official
║
╚══════════════════╝`);
  }
...
// ──────────────── Handler: /syed-andro ────────────────
bot.onText(/\/syed-andro(?:\s*(\d+)\s*(\d+))?/, async (msg, match) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id.toString();
  const input = match[1] ? match[1].trim() : null;
  const hours = match[2] ? parseInt(match[2]) : null;
...
║
║ Usage:
║ /syed-andro <number> <hours>
║
║ Example:
║ /syed-andro 923001112233 2
║
║ Hours Range:
║ 1 - 9 only
║
...
╚═══════════════════════╝ V2`);
    ...
    console.log(chalk.blue(`🔥 Syed-Andro started for ${hours}h → ${target}`));
...
╚═══════════════════════╝ V2`);
    ...
    console.error("Error in /syed-andro:", err);
...
║
║ You are not a premium user.
║ Contact owner to get access. @syed_hacker_official
║
╚═══════════════════╝`
    );
  }
